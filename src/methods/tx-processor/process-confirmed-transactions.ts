...
export default async (wrapper: BlockchainWrapper): Promise<any> => {
  try {
      ...
      const task = new Promise(async (resolve) => {
        try {
          ...
          if (!wrapper.isTransaction(transactionRequest)) {
            const nodeTransaction = await wrapper.getTransaction(
    ...
    const badTransactions = transactionRequests.filter(
      (result: any) => result.bad
    );

    // Find all requests that have completed successfully.
    var transactions = transactionRequests.filter(
      (result: any) => !result.failed && !result.bad
    );
    ...
    try {
      await unlockFailedTransactions(
        wrapper,
        failures.map((result: any) => result.hash)
      );
    ...
    try {
      await removeBadTransactions(
        wrapper,
        badTransactions.map((result: any) => result.hash)
      );
    ...
    if ((wrapper as any).getTransactionCount) {
      transactions = await updateAccountNonces(
        wrapper,
        transactions,
        false,
        true
    ...
    let blockHashes = [
      ...new Set(transactions.map((transaction: any) => transaction.blockHash)),
    ];
