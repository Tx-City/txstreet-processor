...
if (process.env.BCH_NODE)
  console.log("working node BCH ip: ", process.env.BCH_NODE);

if (process.env.USE_DATABASE === "true") mongodb();
...
if (chainsToSubscribe.length == 0)
  chainsToSubscribe = JSON.parse(process.env.CHAINS).map((ticker: string) =>
    ticker.toUpperCase()
  );
...
const getLatestBlockLoop = async (wrapper: any) => {
  ...
  try {
    ...
    if (!isNaN(height) && height > 100) {
      ...
      const heightExistsInDb = await database
        .collection("blocks")
        .find({ chain: wrapper.ticker, height })
        .project({ height: 1 })
        .limit(1)
        .toArray();
...
const init = async () => {
  ...
  if (chainsToSubscribe.includes("LUKSO")) {
    ...
    let luksoWrapper = new wrapperClass.default(
