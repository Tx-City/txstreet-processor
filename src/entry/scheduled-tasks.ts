...
dotenv.config();
console.log("starting scheduledtasks");
...
const run = async () => {
  ...
  if (chains.includes("ETH")) {
    executeJob(
      () => ethRecentContracts("ETH", "5min", Date.now() - minutes(5)),
      seconds(5)
    );
    executeJob(
      () => ethRecentContracts("ETH", "1hour", Date.now() - hours(1)),
      hours(1)
    );
    executeJob(
      () => ethRecentContracts("ETH", "1day", Date.now() - days(1)),
      days(1)
    );
  }
