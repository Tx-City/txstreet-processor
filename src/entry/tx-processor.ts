...
const run = async () => {
  if (nodesToInit.includes("BTC")) {
    ...
    if (process.env.PROCESS_PENDING == "true") processPending(btcWrapper);
    if (process.env.PROCESS_CONFIRMED == "true") processConfirmed(btcWrapper);
  }
  ...
  if (nodesToInit.includes("DASH")) {
    const dashWrapper = new Wrappers.DASHWrapper(
      {
        ...
        port: Number(process.env.DASH_NODE_PORT) || 9998,
      },
      {
        host: process.env.DASH_NODE as string,
    ...
    if (process.env.PROCESS_PENDING == "true") processPending(dashWrapper);
    if (process.env.PROCESS_CONFIRMED == "true") processConfirmed(dashWrapper);
  }
  ...
  if (nodesToInit.includes("BCH")) {
    ...
    if (process.env.PROCESS_PENDING == "true") processPending(bchWrapper);
    if (process.env.PROCESS_CONFIRMED == "true") processConfirmed(bchWrapper);
  }

  if (nodesToInit.includes("XMR")) {
    const xmrWrapper = new Wrappers.XMRWrapper(process.env.XMR_NODE as string);
    if (process.env.PROCESS_PENDING == "true") processPending(xmrWrapper);
    if (process.env.PROCESS_CONFIRMED == "true") processConfirmed(xmrWrapper);
  }
  ...
  if (nodesToInit.includes("LTC")) {
    ...
    if (process.env.PROCESS_PENDING == "true") processPending(ltcWrapper);
    if (process.env.PROCESS_CONFIRMED == "true") processConfirmed(ltcWrapper);
  }

  if (nodesToInit.includes("ETH")) {
    const ethWrapper = new Wrappers.ETHWrapper(process.env.ETH_NODE as string);
    if (process.env.PROCESS_PENDING == "true") processPending(ethWrapper);
    if (process.env.PROCESS_CONFIRMED == "true") processConfirmed(ethWrapper);
  }
  ...
  if (nodesToInit.includes("LUKSO")) {
    ...
    if (process.env.PROCESS_PENDING == "true") processPending(luksoWrapper);
    if (process.env.PROCESS_CONFIRMED == "true") processConfirmed(luksoWrapper);
  }
  ...
  if (nodesToInit.includes("CELO")) {
    ...
    if (process.env.PROCESS_PENDING == "true") processPending(celoWrapper);
    if (process.env.PROCESS_CONFIRMED == "true") processConfirmed(celoWrapper);
  }
