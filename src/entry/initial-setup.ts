...
const createIndexes = async (): Promise<boolean> => {
  try {
    ...
    if (process.env.USE_DATABASE !== "true") return true;
    ...
    try {
      await database
        .collection("blocks")
        .createIndex(
          { chain: 1, broadcast: 1, stored: 1, height: 1 },
          { name: "chain_broadcast_stored_height" }
        );
    } catch (e) {
      console.log(e);
    }
    try {
      await database
        .collection("blocks")
        .createIndex(
          { chain: 1, processed: 1, timestamp: 1 },
          { name: "chain_processed_timestamp" }
        );
    } catch (e) {
      console.log(e);
    }
    try {
      await database.collection("blocks").createIndex(
        { chain: 1, hash: 1 },
        {
          name: "chain_hash",
          unique: true,
    ...
    try {
      await database
        ...
        .createIndex(
          { insertedAt: 1 },
          { name: "time_to_live", expireAfterSeconds: 1209600 }
        );
    } catch (e) {
      console.log(e);
    }
    ...
    try {
      await database
        .collection("contracts_ETH")
        .createIndex({ contract: 1 }, { name: "contract_1", unique: true });
    } catch (e) {
      console.log(e);
    }
    try {
      await database
        ...
        .createIndex(
          { lastUpdated: 1 },
          { name: "TTL_lastUpdated", expireAfterSeconds: 604800 }
        );
    } catch (e) {
      console.log(e);
    }
    ...
    try {
      await database
        .collection("moonhead_owners")
        .createIndex({ address: 1 }, { name: "address" });
    } catch (e) {
      console.log(e);
    }
    try {
      await database
        .collection("moonhead_owners")
        .createIndex({ tokenId: 1 }, { name: "tokenId" });
    } catch (e) {
      console.log(e);
    }
    ...
    try {
      await database
        ...
        .createIndex({ chain: 1 }, { name: "chain", unique: true });
    } catch (e) {
      console.log(e);
    }
    ...
    try {
      await database
        ...
        .updateOne(
          { chain: "ETH-nohistory" },
          { $setOnInsert: { chain: "ETH-nohistory" } },
          { upsert: true }
        );
    } catch (e) {
      console.log(e);
    ...
    try {
      await database
        .collection("statistics")
        .updateOne(
          { chain: "LUKSO-nohistory" },
          { $setOnInsert: { chain: "LUKSO-nohistory" } },
          { upsert: true }
        );
    } catch (e) {
      console.log(e);
    ...
    try {
      await database
        .collection("statistics")
        .updateOne(
          { chain: "CELO-nohistory" },
          { $setOnInsert: { chain: "CELO-nohistory" } },
          { upsert: true }
        );
    } catch (e) {
      console.log(e);
    ...
    try {
      await database
        ...
        .createIndex(
          { chain: 1, interval: 1, created: -1 },
          { name: "chain_interval_created" }
        );
    } catch (e) {
      console.log(e);
    }
    try {
      await database
        .collection("statistics_history")
        .createIndex({ expires: 1 }, { name: "TTL", expireAfterSeconds: 0 });
    } catch (e) {
      console.log(e);
    }
    ...
    try {
      await database
        .collection("statistics_history_snapshots")
        .createIndex({ chain: 1, interval: 1 }, { name: "chain_interval" });
    } catch (e) {
      console.log(e);
    }
    ...
    try {
      await database
        .collection("transactions_BTC")
        .createIndex(
          { confirmed: 1, processed: 1, blockHeight: 1, dropped: 1, fee: -1 },
          { name: "pending_txlist" }
        );
    } catch (e) {
      console.log(e);
    ...
    try {
      await database.collection("transactions_ETH").createIndex(
        {
          confirmed: 1,
          processed: 1,
          blockHeight: 1,
    ...
    try {
      await database
        .collection("transactions_ETH")
        .createIndex({ contract: 1, insertedAt: -1, to: 1 });
    } catch (e) {
      console.log(e);
    }
    try {
    ...
    //LUKSO
    try {
      await database.collection("transactions_LUKSO").createIndex(
        {
          confirmed: 1,
          processed: 1,
          blockHeight: 1,
    ...
    //CELO
    try {
      await database.collection("transactions_CELO").createIndex(
        {
          confirmed: 1,
          processed: 1,
          blockHeight: 1,
    ...
    for (let i = 0; i < txCollections.length; i++) {
      ...
      try {
        await collection.createIndex({ processed: 1, insertedAt: 1 });
      } catch (e) {
        console.log(e);
      }
      ...
      try {
        await collection.createIndex(
          { insertedAt: 1 },
          { name: "TTL", expireAfterSeconds: 1209600 }
        );
      } catch (e) {
        console.log(e);
      }
