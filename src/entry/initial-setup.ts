...
const createIndexes = async (): Promise<boolean> => {
    try {
        ...
        const collectionNames: string[] = ['transactions_BTC', 'transactions_ETH', 'transactions_LUKSO', 'transactions_SOLANA', 'transactions_CELO', 'transactions_LTC', 'transactions_BCH', 'transactions_XMR', 'blocks', 'contracts_ETH', 'moonhead_owners', 'statistics', 'statistics_history', 'statistics_history_snapshots'];
        ...
        const txCollections: any[] = [
            ...
            database.collection('transactions_SOLANA'),
        ...
        try { await database.collection('statistics').updateOne({ "chain": "SOLANA" }, { $setOnInsert: { "chain": "SOLANA" } }, { upsert: true }) } catch (e) { console.log(e) }
        ...
        try { await database.collection('statistics').updateOne({ "chain": "SOLANA-nohistory" }, { $setOnInsert: { "chain": "SOLANA-nohistory" } }, { upsert: true }) } catch (e) { console.log(e) }
