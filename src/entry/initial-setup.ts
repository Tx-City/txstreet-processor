...
const createIndexes = async (): Promise<boolean> => {
    try {
        ...
        const collectionNames: string[] = ['transactions_BTC', 'transactions_ETH', 'transactions_LUKSO', 'transactions_LUMIA', 'transactions_FLR', 'transactions_CELO', 'transactions_LTC', 'transactions_BCH', 'transactions_XMR', 'blocks', 'contracts_ETH', 'moonhead_owners', 'statistics', 'statistics_history', 'statistics_history_snapshots'];
        ...
        try { await database.collection('statistics').updateOne({ "chain": "LUMIA-nohistory" }, { $setOnInsert: { "chain": "LUMIA-nohistory" } }, { upsert: true }) } catch (e) { console.log(e) }
        ...
        //LUMIA
        try { await database.collection('transactions_LUMIA').createIndex({ confirmed: 1, processed: 1, blockHeight: 1, lastProcessed: -1, dropped: 1, pendingSortPrice: -1 }, { name: 'pending_txlist' }); } catch (e) { console.log(e) }
        // try { await database.collection('transactions_ETH').createIndex({ contract: 1, to: 1, timestamp: -1 }); } catch (e) { console.log(e) }
        try { await database.collection('transactions_LUMIA').createIndex({ contract: 1, insertedAt: -1, to: 1 }); } catch (e) { console.log(e) }
        try { await database.collection('transactions_LUMIA').createIndex({ from: 1, nonce: 1 }); } catch (e) { console.log(e) }
        // try { await database.collection('transactions_ETH').createIndex({ house: 1, insertedAt: 1 }); } catch (e) { console.log(e) }
        try { await database.collection('transactions_LUMIA').createIndex({ lastInsert: 1 }, { name: 'TTL_lastInsert', expireAfterSeconds: 86400, partialFilterExpression: { "confirmed": false } }); } catch (e) { console.log(e) }
        try { await database.collection('transactions_LUMIA').createIndex({ confirmed: 1, processed: 1, locked: 1, blockHeight: 1, lastProcessed: 1, timestamp: 1, processFailures: 1, dropped: 1 }, { name: 'general_purpose' }); } catch (e) { console.log(e) }
