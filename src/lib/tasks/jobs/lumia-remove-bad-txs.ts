...
import { LUMIAWrapper } from '../../../lib/node-wrappers'; 
...
export default async (chain: string): Promise<void> => {
    try {
        const node = new LUMIAWrapper(process.env.LUMIA_NODE as string);
        ...
        if(writeInstructions.length > 0) {
            console.log("lumia bad tx removing " + deleteTxHashes.length, ExecutionType[_currentExecutionType]);
                        ...
                        if(!dbBlock) {
                            blockInstructions.push({
                                updateOne: {
                                    ...
                                    update: { $set: { processed: false, locked: false, note: '[cronjob]: lumia-remove-bad-txs' },  $setOnInsert: { timestamp: Date.now(), insertedAt: new Date(), lastInsert: new Date(), processFailures: 0, processMetadata: true, processTransactions: true } },
                                    upsert: true 
                                }  
                            });
                        }
