...
export default async (chain: string): Promise<void> => {
    try {
        const node = new wrappers[chain + "Wrapper"]({ username: 'user', password: 'pass', host: process.env[chain + "_NODE"], port: chain === "LTC" ? Number(process.env.LTC_NODE_PORT) : Number(process.env[chain + "_NODE_PORT"]) || 8332 });
        ...
        await node.rpc.getrawmempool(false, async (err: string, result: any) => {
