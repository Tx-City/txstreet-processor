...
export default async (chain: string): Promise<void> => {
    try {
        ...
        if(writeInstructions.length > 0) {
            console.log("lukso bad tx removing " + deleteTxHashes.length, ExecutionType[_currentExecutionType]);
                        ...
                        if(!dbBlock) {
                            blockInstructions.push({
                                updateOne: {
                                    ...
                                    update: { $set: { processed: false, locked: false, note: '[cronjob]: lukso-remove-bad-txs' },  $setOnInsert: { timestamp: Date.now(), insertedAt: new Date(), lastInsert: new Date(), processFailures: 0, processMetadata: true, processTransactions: true } },
                                    upsert: true 
                                }  
                            });
                        }
