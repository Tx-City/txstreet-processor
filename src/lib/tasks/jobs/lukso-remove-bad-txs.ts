...
import { LUKSOWrapper } from "../../../lib/node-wrappers";
...
const getQueryForExecutionType = (
  chain: string,
  ...
  switch (executionType) {
    case ExecutionType.Custom:
      where = {
        ...
        confirmed: false,
        blockHeight: { $eq: null },
        lastProcessed: { $lte: fifteenSeconds },
...
export default async (chain: string): Promise<void> => {
  try {
    const node = new LUKSOWrapper(process.env.LUKSO_NODE as string);
    ...
    if (executionType == ExecutionType.Large) {
      ...
      query = collection.find(
        queryInstructions.where,
        queryInstructions.project
      );
      if (Object.keys(queryInstructions.sort).length)
        query = query.sort(queryInstructions.sort);
    ...
    if (writeInstructions.length > 0) {
      console.log(
        "lukso bad tx removing " + deleteTxHashes.length,
        ExecutionType[_currentExecutionType]
      );
      redis.publish(
        "removeTx",
        JSON.stringify({ chain, hashes: deleteTxHashes })
      );
                  ...
                  updateOne: {
                    ...
                    update: {
                      $set: {
                        processed: false,
                        locked: false,
                        note: "[cronjob]: lukso-remove-bad-txs",
                      },
                      $setOnInsert: {
                        timestamp: Date.now(),
                        insertedAt: new Date(),
