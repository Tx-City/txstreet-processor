  ...
  try {
        ...
        },
        {
          $project: {
            ...
            weightedTransactions: {
              $add: ["$txCount", { $ifNull: ["$contract.weight", 0] }],
            },
          },
        },
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (
        !result.contract ||
        result.nameless ||
        (result?.lastChecked && Date.now() - result.lastChecked > 300000)
      ) {
        const promise = limiter.schedule(async () => {
          try {
            let response = await axios.get(
              `https://api.etherscan.io/api?module=contract&action=getsourcecode&address=${result._id}&apiKey=${process.env.ETHERSCAN_API_KEY}`
            );
            if (response.status === 200) {
              ...
              if (
                ...
                response = await axios.get(
                  `https://api.etherscan.io/api?module=account&action=txlist&address=${result._id}&startblock=0&endblock=99999999&page=1&sort=asc&apikey=${process.env.ETHERSCAN_API_KEY}`
                );
                if (
                  response.status === 200 &&
    ...
    await database
      .collection(`contracts_${chain}`)
      .updateMany(
        { contract: { $nin: exclude }, weight: { $lte: -0.01 } },
        { $inc: { weight: 0.01 } }
      );
    ...
    values = values.sort(
      (a: any, b: any) => b.weightedTransactions - a.weightedTransactions
    );
    // values.forEach((v: any) => delete v.weightedTransactions);
    await storeObject(
      path.join("live", `trending-contracts-${chain}-${label}`),
