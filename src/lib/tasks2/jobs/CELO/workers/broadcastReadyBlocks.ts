...
setInterval(async () => {
    try {
        ...
        const blocks = await collection.find({ chain: 'CELO', processed: true, txsChecked: true, broadcast: false, hash: { $ne: null } }).sort({ height: 1 }).limit(1).toArray();
...
const storeBlock = async (database: any, block: any) => {
    try {
        const remainingTxs = await database.collection('transactions_CELO').find({ confirmed: false, blockHash: block.hash, dropped: { $exists: false } }).count();
        if (remainingTxs > 0) {
            let remainingFull = await database.collection('transactions_CELO').find({ confirmed: false, blockHash: block.hash, dropped: { $exists: false } }).limit(20).toArray();
            for (let i = 0; i < remainingFull.length; i++) {
                const tx = remainingFull[i];
                if(tx.locked && Date.now() - tx.lockedAt > 3000){
                    await database.collection('transactions_CELO').updateOne({ hash: tx.hash }, { $set: { locked: false, processed: false } });
                }
            }
        ...
        block.txFull = {};
        const transactions = await database.collection('transactions_CELO').find({ hash: { $in: block.transactions }, confirmed: true }).toArray();
        if(block.transactions && block.transactions.length > 0 && transactions.length === 0){
            for (let i = 0; i < block.transactions.length; i++) {
                const hash = block.transactions[i];
                await database.collection('transactions_CELO').updateOne({ hash }, { $set: { blockHash: block.hash, blockHeight:block.height, blockNumber: block.number, confirmed: true, processed: false, locked: false, processFailures: 0, lastInsert: new Date(), insertedAt: new Date() }, $unset: { dropped: "" } }, { upsert: true });
            }
            return false;
        }
        ...
        transactions.forEach((transaction: any) => {
            const formatted = formatTransaction('CELO', transaction);
        ...
        const formattedBlock: any = formatBlock('CELO', block);
        ...
        await storeObject(path.join('blocks', 'CELO', firstPart, secondPart, block.hash), fileContents);
        // await calculateBlockStats(block, transactions);
        await database.collection('blocks').updateOne({ chain: 'CELO', hash: block.hash }, { $set: { stored: true, broadcast: false } });
        block.stored = true;
...
const checkBlock = async (database: any, block: any, depth: number = 0) => {
    ...
    try {
        ...
        if (block.parentHash && depth === 0)
            parent = await database.collection('blocks').findOne({ chain: 'CELO', hash: block.parentHash });
        ...
        if (block.stored && parent && parent.stored || block.stored && !parent) {
            let uncles = await database.collection('blocks').find({ chain: 'CELO', hash: { $in: block.uncles || [] } });
            ...
            redis.publish('block', JSON.stringify({ chain: 'CELO', height: block.height, hash: block.hash }));
            await database.collection('blocks').updateOne({ chain: 'CELO', hash: block.hash }, { $set: { broadcast: true, note: 'broadcast-ready-block' } });
            return true;
        } else {
