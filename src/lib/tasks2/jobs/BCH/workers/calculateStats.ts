...
setInterval(async () => {
    try {
        ...
        initTasks.push(new Promise((resolve, reject) => {
            database.collection('statistics').findOne({ chain: 'BCH' }, { fiatPrice: 1 })
                .then((document: any) => {
        ...
        initTasks.push(new Promise((resolve, reject) => {
            const dataPath = path.join(__dirname, '..', '..', '..', '..', '..', 'data', 'transactions-BCH.bin'); 
        ...
        initTasks.push(new Promise((resolve, reject) => {
            const dataPath = path.join(__dirname, '..', '..', '..', '..', '..', 'data', 'blocks-BCH.bin'); 
            fs.readFile(dataPath,  (err: NodeJS.ErrnoException, data: Buffer) => {
                if(err) return reject(err); 
        ...
        try { lastExecutionResults['medianFee-satPerByte'] = (await medianFeeSatPerByte(transactions)) as number; } catch (error) { Logger.error(error); };
