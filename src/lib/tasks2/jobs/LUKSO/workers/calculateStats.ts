...
import {
  ETHBlocksSchema,
  ETHTransactionsSchema,
} from "../../../../../data/schemas";
import {
  ProjectedEthereumBlock,
  ProjectedEthereumTransaction,
} from "../../../types";
...
const interval = setInterval(async () => {
  try {
    ...
    initTasks.push(
      new Promise((resolve, reject) => {
        database
          ...
          .findOne({ chain: "LUKSO" }, { fiatPrice: 1 })
          .then((document: any) => {
      ...
      new Promise((resolve, reject) => {
        ...
        fs.readFile(dataPath, (err: NodeJS.ErrnoException, data: Buffer) => {
          ...
          try {
            ...
            transactions = parsed.collection.filter(
              (transaction: ProjectedEthereumTransaction) =>
                transaction.insertedAt >= lowerRange &&
                transaction.insertedAt <= upperRange
            );
            transactions = transactions.sort(
              (
                a: ProjectedEthereumTransaction,
                b: ProjectedEthereumTransaction
            ...
            try {
              console.log(
                `Decoded information for error:`,
                ETHTransactionsSchema.decode(data)
              );
            } catch (error) {
              console.log("Schema could not be decoded.");
            }
            return reject(error);
          }
        });
      ...
      new Promise((resolve, reject) => {
        ...
        fs.readFile(dataPath, (err: NodeJS.ErrnoException, data: Buffer) => {
          ...
          try {
            ...
            // Filter the collection to obtain the transactions within the specified range.
            last250Blocks = parsed.collection
              .sort(
                (a: ProjectedEthereumBlock, b: ProjectedEthereumBlock) =>
                  a.height - b.height
              )
              .slice(0, 250);
            blocks = parsed.collection.filter(
              (block: ProjectedEthereumBlock) =>
    ...
    try {
      lastExecutionResults["tps"] = await tps(transactions);
    } catch (error) {
      console.error(error);
    }
    try {
      lastExecutionResults["ctps"] = await ctps(blocks);
    } catch (error) {
      console.error(error);
    }
    try {
      lastExecutionResults["medianBlockSize"] = await medianBlockSize(blocks);
    } catch (error) {
      console.error(error);
    }
    try {
      lastExecutionResults["medianBlockTime"] = await medianBlockTime(
        last250Blocks
      );
    } catch (error) {
      console.error(error);
    ...
    try {
      lastExecutionResults["medianTxsPerBlock"] = await medianTxsPerBlock(
        blocks
      );
    } catch (error) {
      console.error(error);
    ...
    try {
      lastExecutionResults["blockHeight"] = await blockHeight(lastKnownBlock);
    } catch (error) {
      console.error(error);
    }
    try {
      lastExecutionResults["difficulty"] = (await difficulty(
        lastKnownBlock
      )) as string;
    } catch (error) {
      console.error(error);
    ...
    try {
      lastExecutionResults["gasUsedDif"] = await gasUsedDif(blocks);
    } catch (error) {
      console.error(error);
    }
    // try { lastExecutionResults['tipPrice'] = await tipPrice(lastKnownBlock); } catch (error) { console.error(error) }
    try {
      lastExecutionResults["baseFee"] = await baseFee(lastKnownBlock);
    } catch (error) {
      console.error(error);
    }
    try {
      lastExecutionResults["gasTarget"] = await gasTarget(lastKnownBlock);
    } catch (error) {
      console.error(error);
    }
    try {
      lastExecutionResults["gasLimit"] = await gasLimit(lastKnownBlock);
    } catch (error) {
      console.error(error);
    }
    try {
      lastExecutionResults["medianGasUsed"] = await medianGasUsed(blocks);
    } catch (error) {
      console.error(error);
    }
    try {
      lastExecutionResults["medianFee-gasPrice"] = await medianFeeGasPrice(
        transactions
      );
    } catch (error) {
      console.error(error);
    ...
    try {
      lastExecutionResults["medianFee-usd"] = await medianFeeUsd(
        transactions,
        pricePerIncrement,
        lastExecutionResults["gasUsedDif"]
      );
    } catch (error) {
      console.error(error);
    }
    try {
      lastExecutionResults["medianFee-usdTransfer"] =
        await medianFeeUsdTransfer(
          pricePerIncrement,
          lastExecutionResults["medianFee-gasPrice"]
        );
    } catch (error) {
      console.error(error);
    }
    ...
    try {
      ...
      if (process.env.UPDATE_DATABASES.toLowerCase() == "true") {
        ...
        await collection.updateOne(
          { chain: "LUKSO" },
          { $set: lastExecutionResults }
        );
        redis.publish(
          "stats",
          JSON.stringify({ chain: "LUKSO", ...lastExecutionResults })
        );
      } else {
        console.log("=========================");
