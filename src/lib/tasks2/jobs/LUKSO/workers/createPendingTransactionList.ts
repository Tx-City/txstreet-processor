...
setInterval(async () => {
  try {
    ...
    let transactions = parsed.collection.sort(
      (a: ProjectedEthereumTransaction, b: ProjectedEthereumTransaction) =>
        (b.maxFeePerGas || b.gasPrice) - (a.maxFeePerGas || a.gasPrice)
    );
    ...
    let uniqueAccounts: string[] = [
      ...new Set(
        transactions.map(
          (transaction: ProjectedEthereumTransaction) => transaction.from
        )
      ),
    ] as string[];
    ...
    let qResult = await database
      .collection("transactions_LUKSO")
      .find({ hash: { $in: requestHashes } })
      .project({ _id: 0, hash: 1, to: 1, house: 1, nonce: 1 })
      .toArray();
    ...
    //TMP
    let _remove = await database
      .collection("transactions_LUKSO")
      .find({ hash: { $in: hashes }, blockHash: { $ne: null } })
      .project({ hash: 1 })
      .toArray();
    ...
    transactions = await updateAccountNonces(luksoWrapper, transactions);
    ...
    for (let i = 0; i < transactions.length; i++) {
      ...
      // The next nonce is the currentTransactionCount + the amount of transactions added.
      const nextNonce =
        (addedByAddress[transaction.from] || 0) + (transaction.fromNonce || 0);

      if (nextNonce === transaction.nonce) {
        pushAndCheckToMove(transaction);
      } else {
        if (!toMove[transaction.from]) toMove[transaction.from] = [];
    ...
    if (count > 0)
      console.log(
        `Found ${count} accounts without a fromNonce in pending transaction creation`
      );
    ...
    if (Date.now() - lastUploadTime >= 1990) {
      ...
      await storeObject(path.join("live", `pendingTxs-LUKSO`), content);
    }
  } catch (error) {
    console.error(error);
  }
