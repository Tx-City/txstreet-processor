...
setInterval(async () => {
    try {
        const dataPath = path.join(__dirname, '..', '..', '..', '..', '..', 'data', 'LUKSO-pendingTransactions.bin');
        ...
        let qResult = await database.collection('transactions_LUKSO').find({ hash: { $in: requestHashes } }).project({ _id: 0, hash: 1, to: 1, house: 1, nonce: 1 }).toArray();
        ...
        //TMP
        let _remove = await database.collection('transactions_LUKSO').find({ hash: { $in: hashes }, blockHash: { $ne: null } }).project({ hash: 1 }).toArray();
