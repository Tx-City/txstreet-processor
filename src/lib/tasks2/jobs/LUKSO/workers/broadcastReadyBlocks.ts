...
const dataDir = path.join(
  (process.env.DATA_DIR as string) || "/mnt/disks/txstreet_storage"
);
...
const storeBlock = async (database: any, block: any) => {
  try {
    const remainingTxs = await database
      ...
      .count();
    if (remainingTxs > 0) {
      let remainingFull = await database
        .collection("transactions_LUKSO")
        .find({
          confirmed: false,
          blockHash: block.hash,
      ...
      for (let i = 0; i < remainingFull.length; i++) {
        ...
        if (tx.locked && Date.now() - tx.lockedAt > 3000) {
          await database
            ...
            .updateOne(
              { hash: tx.hash },
              { $set: { locked: false, processed: false } }
            );
        }
      }
      console.log(
        `Block ${block.hash} is still waiting on ${remainingTxs} transactions to be processed.`
      );
      return false;
    }
    ...
    const transactions = await database
      .collection("transactions_LUKSO")
      .find({ hash: { $in: block.transactions }, confirmed: true })
      .toArray();
    if (
      block.transactions &&
    ...
    console.log(
      ...
      block.hash,
      "TxFull",
      Object.values(block.txFull).length,
      "Transactions:",
      transactions.length,
      "Block transactions:",
    ...
    await storeObject(
      path.join("blocks", "LUKSO", firstPart, secondPart, block.hash),
      fileContents
    );
    ...
    await database
      .collection("blocks")
      .updateOne(
        { chain: "LUKSO", hash: block.hash },
        { $set: { stored: true, broadcast: false } }
      );
    block.stored = true;
...
const checkBlock = async (database: any, block: any, depth: number = 0) => {
  ...
  try {
    ...
    if (!block.stored) {
      ...
      if (!(await storeBlock(database, block))) return false;
    }
    ...
    if (block.parentHash && depth === 0)
      parent = await database
        .collection("blocks")
        .findOne({ chain: "LUKSO", hash: block.parentHash });
    ...
    if (parent) {
      if (!(await checkBlock(database, parent, depth + 1))) return false;
    }

    // If the previous block passed all checks (or doesn't exist, sanity, depth limit).
    if (
      (block.stored && parent && parent.stored) ||
      (block.stored && !parent)
    ) {
      let uncles = await database
        .collection("blocks")
        .find({ chain: "LUKSO", hash: { $in: block.uncles || [] } });
      ...
      await database
        ...
        .updateOne(
          { chain: "LUKSO", hash: block.hash },
          { $set: { broadcast: true, note: "broadcast-ready-block" } }
        );
      return true;
    } else {
