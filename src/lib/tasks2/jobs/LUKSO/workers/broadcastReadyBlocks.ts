...
setInterval(async () => {
    try {
        ...
        const blocks = await collection.find({ chain: 'LUKSO', processed: true, txsChecked: true, broadcast: false, hash: { $ne: null } }).sort({ height: 1 }).limit(1).toArray();
...
const storeBlock = async (database: any, block: any) => {
    try {
        const remainingTxs = await database.collection('transactions_LUKSO').find({ confirmed: false, blockHash: block.hash, dropped: { $exists: false } }).count();
        if (remainingTxs > 0) {
            let remainingFull = await database.collection('transactions_LUKSO').find({ confirmed: false, blockHash: block.hash, dropped: { $exists: false } }).limit(20).toArray();
            for (let i = 0; i < remainingFull.length; i++) {
                const tx = remainingFull[i];
                if(tx.locked && Date.now() - tx.lockedAt > 3000){
                    await database.collection('transactions_LUKSO').updateOne({ hash: tx.hash }, { $set: { locked: false, processed: false } });
                }
            }
        ...
        block.txFull = {};
        const transactions = await database.collection('transactions_LUKSO').find({ hash: { $in: block.transactions }, confirmed: true }).toArray();
        if(block.transactions && block.transactions.length > 0 && transactions.length === 0){
            for (let i = 0; i < block.transactions.length; i++) {
                const hash = block.transactions[i];
                await database.collection('transactions_LUKSO').updateOne({ hash }, { $set: { blockHash: block.hash, blockHeight:block.height, blockNumber: block.number, confirmed: true, processed: false, locked: false, processFailures: 0, lastInsert: new Date(), insertedAt: new Date() }, $unset: { dropped: "" } }, { upsert: true });
            }
            return false;
        }
        ...
        transactions.forEach((transaction: any) => {
            const formatted = formatTransaction('LUKSO', transaction);
        ...
        const formattedBlock: any = formatBlock('LUKSO', block);
        ...
        await storeObject(path.join('blocks', 'LUKSO', firstPart, secondPart, block.hash), fileContents);
        // await calculateBlockStats(block, transactions);
        await database.collection('blocks').updateOne({ chain: 'LUKSO', hash: block.hash }, { $set: { stored: true, broadcast: false } });
        block.stored = true;
...
const checkBlock = async (database: any, block: any, depth: number = 0) => {
    ...
    try {
        ...
        if (block.parentHash && depth === 0)
            parent = await database.collection('blocks').findOne({ chain: 'LUKSO', hash: block.parentHash });
        ...
        if (block.stored && parent && parent.stored || block.stored && !parent) {
            let uncles = await database.collection('blocks').find({ chain: 'LUKSO', hash: { $in: block.uncles || [] } });
            ...
            redis.publish('block', JSON.stringify({ chain: 'LUKSO', height: block.height, hash: block.hash }));
            await database.collection('blocks').updateOne({ chain: 'LUKSO', hash: block.hash }, { $set: { broadcast: true, note: 'broadcast-ready-block' } });
            return true;
        } else {
