...
setInterval(async () => {
    try {
        ...
        const lastPost = await database.collection('transactions_ETH').find({from:"0x8f2d2da3044b0a1ea54ee26f7fe376cd9ec4393f"}).sort({timestamp: -1}).limit(1).toArray();
        const lastPostTx = lastPost[0];
        // console.log(lastPost, "lastPost");

        const dataPath = path.join(__dirname, '..', '..', '..', '..', '..', 'data', 'transactions-LUMIA.bin');
        ...
        // console.log(hashes.length, "lumia txs");

        database.collection('statistics').updateOne({ chain: 'LUMIA' },{ $set: {pendingBatchCount: hashes.length }});
        // transactions = transactions.filter((tx: any) => !_remove.includes(tx.hash));


        if (Date.now() - lastUploadTime >= 1990) {
            // const _path = path.join(__dirname, '..', '..', '..', '..', '..', 'data', 'LUMIA-pendingBatch.json');
            ...
            await storeObject(path.join('live', `pendingBatch-LUMIA`), JSON.stringify(hashes));
        }
