...
const interval = setInterval(async () => {
    try {
        ...
        initTasks.push(new Promise((resolve, reject) => {
            database.collection('statistics').findOne({ chain: 'LUMIA' }, { fiatPrice: 1 })
                .then((document: any) => {
        ...
        initTasks.push(new Promise((resolve, reject) => {
            const dataPath = path.join(__dirname, '..', '..', '..', '..', '..', 'data', 'blocks-LUMIA.bin'); 
            fs.readFile(dataPath,  (err: NodeJS.ErrnoException, data: Buffer) => {
                if(err) return reject(err); 
        ...
        try {
            ...
            if(process.env.UPDATE_DATABASES.toLowerCase() == "true") {
                ...
                await collection.updateOne({ chain: 'LUMIA' }, { $set: lastExecutionResults }); 
                redis.publish('stats', JSON.stringify({ chain: "LUMIA", ...lastExecutionResults })); 
            } else {
                console.log('=========================')
