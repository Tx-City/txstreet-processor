...
setInterval(async () => {
    try {
        ...
        const { baseFee, blockHeight, gasUsedDif, gasLimit } = await database.collection('statistics')
            .findOne({ chain: 'LUMIA' }, { baseFee: 1, blockHeight: 1, gasUsedDif: 1, gasLimit: 1 });
        ...
        if(process.env.UPDATE_DATABASES === "true") {
            await database.collection('statistics').updateOne({ chain: "LUMIA" }, { $set: { blockGasEstimates: value } }); 
        }
    } catch (error) {
        /* ignored */
        console.log(error);
