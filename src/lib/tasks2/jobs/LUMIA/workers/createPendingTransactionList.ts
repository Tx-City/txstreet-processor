...
const lumiaWrapper = new Wrappers.LUMIAWrapper(process.env.LUMIA_NODE as string);
...
setInterval(async () => {
    try {
        const dataPath = path.join(__dirname, '..', '..', '..', '..', '..', 'data', 'LUMIA-pendingTransactions.bin');
        ...
        let qResult = await database.collection('transactions_LUMIA').find({ hash: { $in: requestHashes } }).project({ _id: 0, hash: 1, to: 1, house: 1, nonce: 1 }).toArray();
        ...
        //TMP
        let _remove = await database.collection('transactions_LUMIA').find({ hash: { $in: hashes }, blockHash: { $ne: null } }).project({ hash: 1 }).toArray();
        ...
        transactions = await updateAccountNonces(lumiaWrapper, transactions);
        ...
        if (Date.now() - lastUploadTime >= 1990) {
            const _path = path.join(__dirname, '..', '..', '..', '..', '..', 'data', 'LUMIA-pendingTransactions.json');
            ...
            await storeObject(path.join('live', `pendingTxs-LUMIA`), content);
        }
