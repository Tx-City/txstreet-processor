...
setInterval(async () => {
    try {
        ...
        const blocks = await collection.find({ chain: 'ARBI', processed: true, txsChecked: true, broadcast: false, hash: { $ne: null } }).sort({ height: 1 }).limit(1).toArray();
...
const storeBlock = async (database: any, block: any) => {
    try {
        const remainingTxs = await database.collection('transactions_ARBI').find({ confirmed: false, blockHash: block.hash, dropped: { $exists: false } }).count();
        if (remainingTxs > 0) {
        ...
        block.txFull = {};
        const transactions = await database.collection('transactions_ARBI').find({ hash: { $in: block.transactions }, confirmed: true }).toArray();
        console.log(block.transactions.length, transactions.length);
        ...
        transactions.forEach((transaction: any) => {
            const formatted = formatTransaction('ARBI', transaction);
        ...
        const formattedBlock: any = formatBlock('ARBI', block);
        ...
        try { await fs.promises.mkdir(path.join(dataDir, 'blocks', 'ARBI', firstPart, secondPart), { recursive: true }); } catch (err) { }
        await storeObject(path.join('blocks', 'ARBI', firstPart, secondPart, block.hash), fileContents);
        // await calculateBlockStats(block, transactions);
        await database.collection('blocks').updateOne({ chain: 'ARBI', hash: block.hash }, { $set: { stored: true, broadcast: false } });
        block.stored = true;
...
const checkBlock = async (database: any, block: any, depth: number = 0) => {
    ...
    try {
        ...
        if (block.parentHash && depth === 0)
            parent = await database.collection('blocks').findOne({ chain: 'ARBI', hash: block.parentHash });
        ...
        if (block.stored && parent && parent.stored || block.stored && !parent) {
            let uncles = await database.collection('blocks').find({ chain: 'ARBI', hash: { $in: block.uncles || [] } });
            ...
            redis.publish('block', JSON.stringify({ chain: 'ARBI', height: block.height, hash: block.hash }));
            await database.collection('blocks').updateOne({ chain: 'ARBI', hash: block.hash }, { $set: { broadcast: true, note: 'broadcast-ready-block' } });
            return true;
        } else {
