...
setInterval(async () => {
    try {
        const wrapper = new EVOLUTIONWrapper(process.env.EVOLUTION_NODE);
        ...
        try {
            ...
            if(process.env.UPDATE_DATABASES.toLowerCase() == "true") {
                ...
                // await collection.updateOne({ chain: 'EVOLUTION' }, { $set: lastExecutionResults }); 
                // redis.publish('stats', JSON.stringify({ chain: "EVOLUTION", ...lastExecutionResults })); 
            } else {
                console.log('=========================')
