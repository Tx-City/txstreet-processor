    ...
    super(async () => {
      try {
        ...
        const where: any = {
          ...
          timestamp: {
            $gt:
              this._lastKnownItemTimestamp === 0
        ...
        if (results.length < 250 && this._firstExecution) {
          ...
          let earliest = results.sort((a: any, b: any) => b.height - a.height)[
            results.length - 1
          ];
          if (!earliest)
            earliest = (
          ...
          let _results = await collection
            .find({ chain, height: { $lt: earliestHeight } })
            .project(project)
            .sort({ height: -1 })
            .limit(remainder)
            .toArray();
        ...
        for (let i = 0; i < results.length; i++) {
          if (!results[i].transactions) results[i].transactions = [];
          let txcount = results[i].transactions.length;
          delete results[i].transactions;
          results[i].transactions = txcount;
          if (!results[i].gasUsedDif) results[i].gasUsedDif = 0.0;
        }

        if (results.length > 0) {
          const latest = results.sort((a: any, b: any) => a.height - b.height)[
            results.length - 1
          ];
        ...
        this._done = true;
        if (this._firstExecution) this._firstExecution = false;
      } catch (error) {
        console.error(error);
        console.error(error);
      }
