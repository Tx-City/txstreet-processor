...
export default class XMRPendingList {
    ...
    // Index -> Value 
    public array: ProjectedXMRTransaction[] = []; 
    // Key -> Index 
    ...
    constructor() {
        ...
        redis.events.on('pendingTx', async (data) => {
            ...
            const transaction: ProjectedXMRTransaction = {
                hash: data.tx,
                fee: parseFloat(data.aByte) * parseFloat(data.s),
                size: data.s,
     ...
     */
    add(transactions: ProjectedXMRTransaction[]) {
        this.array = this.array.concat(transactions); 
        this.array = this.array.sort((a: ProjectedXMRTransaction, b: ProjectedXMRTransaction) => this._getSortValue(a) - this._getSortValue(b)); 
     ...
     */
    _onPendingTransactions = async (transactions: ProjectedXMRTransaction[]) => {
     ...
     */
    _getSortValue(transaction: ProjectedXMRTransaction) {
