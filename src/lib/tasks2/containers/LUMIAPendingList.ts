...
export default class LUMIAPendingList {
    ...
    constructor() {
        ...
        this._filePath = path.join(__dirname, '..', '..', '..', 'data', 'LUMIA-pendingTransactions.bin');
        ...
        setInterval(async () => {
            ...
            const collection = database.collection('transactions_LUMIA');
        ...
        try {
            ...
            const obtainBlock = async (): Promise<any> => {
                try {
                    ...
                    const filePath = path.join(directory, 'blocks', 'LUMIA', firstPart, secondPart, hash);
            ...
            if (!block) {
                console.log(`LUMIA PendingList Failed to get data for block: ${hash}`);
                return;
            }
