...
import { TONTransactionsSchema } from "../../../data/schemas";

export default class TONPendingList {
  ...
  add(transactions: ProjectedBTCTransaction[]) {
    ...
    this.array = this.array.sort(
      (a: ProjectedBTCTransaction, b: ProjectedBTCTransaction) =>
  ...
  _writeTask = async (): Promise<void> => {
    try {
      ...
      const contents = TONTransactionsSchema.toBuffer({
        timestamp: Date.now(),
