...
export default class CELOPendingList {
    ...
    constructor() {
        ...
        this._filePath = path.join(__dirname, '..', '..', '..', 'data', 'CELO-pendingTransactions.bin');
        ...
        setInterval(async () => {
            ...
            const collection = database.collection('transactions_CELO');
        ...
        try {
            ...
            const obtainBlock = async (): Promise<any> => {
                try {
                    ...
                    const filePath = path.join(directory, 'blocks', 'CELO', firstPart, secondPart, hash);
            ...
            if (!block) {
                console.log(`CELOPendingList Failed to get data for block: ${hash}`);
                return;
            }
