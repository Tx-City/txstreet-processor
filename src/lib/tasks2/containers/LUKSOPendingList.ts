...
import fs from "fs";
import OverlapProtectedInterval, {
  setInterval,
} from "../utils/OverlapProtectedInterval";
import { ETHTransactionsSchema } from "../../../data/schemas";

export default class LUKSOPendingList {
  ...
  constructor() {
    ...
    setInterval(async () => {
      ...
      const collection = database.collection("transactions_LUKSO");
      const hashes = this.array.map((a: any) => a.hash);
      const result = await collection
        .find({
          hash: { $in: hashes },
          $or: [{ blockHash: { $ne: null } }, { dropped: { $exists: true } }],
        })
    ...
    setInterval(async () => {
      ...
      this.array = this.array.sort(
        (a: ProjectedEthereumTransaction, b: ProjectedEthereumTransaction) =>
          this._getSortValue(a) - this._getSortValue(b)
      );
   ...
   */
  _onPendingTransactions = async (
    transactions: ProjectedEthereumTransaction[]
  ) => {
  ...
  _onConfirmedBlock = async (hash: string) => {
    try {
      ...
      const obtainBlock = async (): Promise<any> => {
        try {
          ...
          const directory =
            ...
            path.join("/mnt", "disks", "txstreet_storage");
      ...
      if (!block) {
        console.log(`LUKSOPendingList Failed to get data for block: ${hash}`);
        return;
      }
      if (block.insertedAt)
        block.insertedAt = new Date(block.insertedAt).getTime();
  ...
  _getSortValue(transaction: ProjectedEthereumTransaction) {
    ...
    if (
      transaction.hasOwnProperty("maxFeePerGas") &&
  ...
  _rebuildKeyMap() {
    ...
    // Sort the array
    this.array = this.array.sort(
      (a: any, b: any) => this._getSortValue(a) - this._getSortValue(b)
    );
  ...
  async init() {
    try {
      ...
      this.add(results);
      console.log(
        `Added ${results.length} results from the database, initialization completed`
      );
    ...
    try {
      ...
      for (let i = 0; i < this.array.length; i++) {
        ...
        if (entry.pExtras && typeof entry.pExtras !== "string")
          entry.pExtras = JSON.stringify(entry.pExtras);

        //@ts-ignore
        Object.keys(entry).forEach(
          (k) =>
            (!entry[k] || entry[k] == null || entry[k] == "null") &&
            delete entry[k]
        );
      }

      const contents = ETHTransactionsSchema.toBuffer({
        timestamp: Date.now(),
