...
export default (chain: string, data: any) => {
  ...
  if (
    ...
    if (
      typeof data.extras !== "undefined" &&
      data.extras &&
      Object.keys(data.extras).length > 0
    )
      obj.e = data.extras;
    if (data.house && data.house != "0" && data.house != "0.0")
      obj.h = data.house;
    obj.g = data.gas;
    ...
    obj.tot = +(
      data.value > 0 ? Number(data.value) / Math.pow(10, 18) : 0
    ).toFixed(6);
    ...
    if (data.deletedHashes && data.deletedHashes.length)
      obj.dh = data.deletedHashes || [];

    obj.ty = Number(data.type) || 0;
    if (data.maxFeePerGas) obj.mfpg = Number(data.maxFeePerGas);
    if (data.maxPriorityFeePerGas)
      obj.mpfpg = Number(data.maxPriorityFeePerGas);
    try {
          ...
          if (zoomer.attributes) {
            for (let i = 0; i < zoomer.attributes.length; i++) {
              ...
              if (attr.trait_type === "Clan")
                obj.char = attr.value.toLowerCase();
            }
          }
        }
      }
    ...
    if (data.fees && data.size) {
      obj[chain === "LTC" ? "lpb" : "spb"] = parseFloat(
        ((data.fees.base * 100000000) / data.size).toFixed(2)
      );
    } else if (data.fee && data.size) {
      obj[chain === "LTC" ? "lpb" : "spb"] = parseFloat(
        (data.fee / data.size).toFixed(2)
      );
    } else {
    ...
    if (data.from && data.from.length && data.from.length <= 10)
      obj.fr = data.from;
