...
class DefaultHousing extends ChainImplementation {
  ...
  async init(): Promise<ChainImplementation> {
    try {
      if (process.env.USE_DATABASE === "false") return this;
      const { database } = await mongodb();
      const collection = database.collection("houses");
      const results = await collection
        .find({ chain: this.chain, name: { $nin: ["up"] } })
        .toArray();
      for (let i = 0; i < results.length; i++) {
        let doc = results[i];
        if (!doc.contracts) continue;
        doc.contracts.forEach(
          (address: string) =>
            (this.mapAddressToHouse[address.toLowerCase()] = doc.name)
        );
      }
      console.log("initialized default housing for LUKSO");
    } catch (error) {
      console.error(error);
    } finally {
      return this;
...
export default new DefaultHousing("LUKSO");
