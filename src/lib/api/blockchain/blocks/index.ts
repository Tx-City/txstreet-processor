...
router.get("/:chain/:id", async (request: Request, response: Response) => {
  ...
  if (!chain)
    return response.json({
      success: false,
      code: -1,
      message: "Chain not provided in request.",
    });
  ...
  if (request.params.id.toLowerCase().includes("0x")) isHeight = false;
  ...
  if (!id)
    return response.json({
      success: false,
      code: -1,
      message: "Block id not provided in request.",
    });
  if (isHeight && Number(id) <= 0)
    return response.json({
      success: false,
      code: -1,
      message: "Block id (height) is invalid",
    });
  ...
  if (isHeight) {
    ...
    if (id > topHeight)
      return response.json({
        success: false,
        code: -1,
        message: `Block id (height) is higher than known head.`,
      });
  } else {
  ...
  const existingBlock: any = await collection.findOne({
    [isHeight ? "height" : "hash"]: id,
  });

  // If the block request already exists and has been processed, we simply process the request.
  if (
    existingBlock &&
    existingBlock.processed &&
    existingBlock.lastTransactionFetch > Date.now() - 1209600000
  ) {
    ...
    const directory =
      process.env.DATA_DIR || path.join("/mnt", "disks", "txstreet_storage");
    ...
    return response.json({
      success: false,
      code: 2,
      message: `Block is not servicable`,
    });
  }
