...
router.get("/:chain/:id", async (request: Request, response: Response) => {
  ...
  if (!id || id.length > 100)
    return response.json({
      success: false,
      code: -1,
      message: "Txid not provided in request.",
    });
  ...
  if (existingTx && existingTx.processed) {
    if (existingTx.dropped) {
      return response.json({
        success: false,
        code: 5,
        message: "Transaction has been dropped",
      });
    }
  ...
  if (Object.keys(subscriber.txSubs).length > MAX_CONCURRENT)
    return response.json({
      success: false,
      code: 2,
      message: `The server is currently too busy to process this request.`,
    });

  // Register a callback that will resolve this request.
  subscriber.subscribeToTx(
    id,
    timeout || TIMEOUT_FOR_REQUEST,
    async (error: string, timeout: boolean, data: any) => {
      if (error && !timeout)
