...
export default class LUKSOWrapper extends BlockchainWrapper {
  ...
  public async getTransaction(
    ...
    try {
      ...
      if (transaction.v) transaction.v = Number(transaction.v);
      if (transaction.value) transaction.value = Number(transaction.value);
      if (transaction.maxPriorityFeePerGas)
        transaction.maxPriorityFeePerGas = Number(
          transaction.maxPriorityFeePerGas
        );
      if (transaction.maxFeePerGas)
        transaction.maxFeePerGas = Number(transaction.maxFeePerGas);

      transaction.pendingSortPrice = Number(
      ...
      if (msg.includes("connection not open on send")) process.exit(1);
      console.error(error);
      return null;
    }
  }
  ...
  public async getUncle(
    ...
    try {
      const block: any = await this.web3.eth.getUncle(
        blockId,
        uncleIndex,
        true
      );
