...
export interface BTCZmqConfig {
    host: string,
    port?: number
}

export default class BTCWrapper extends BlockchainWrapper {
    public rpc: any;
    public sock: any;
    public configRpc: BTCRpcConfig;
    public configZmq?: BTCZmqConfig;

    constructor(configRpc: BTCRpcConfig, configZmq?: BTCZmqConfig) {
        super('BTC');
        ...
        this.sock.on('message', (topicBuffer: Buffer, messageBuffer: Buffer) => {
            ...
            switch (topic) {
                ...
                case 'hashblock':
                    ...
                    if (memcache.get(`btc-${hash}`)) return;
                    memcache.put(`btc-${hash}`, 1);
